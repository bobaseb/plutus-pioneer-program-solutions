...
--{-# INLINABLE lovelaces #-}
--numTokens :: AssetClass -> Integer
--numTokens = Ada.getLovelace . Ada.fromValue
...
transition ts s r = case (stateValue s, stateData s, r) of
    ...
    (_, _, Close)                          -> Just (Constraints.mustBeSignedBy (tsSeller ts)
                                                         , State Nothing $
                                                           mempty
                                                         )
    {-(v, Just p, Close)                             ->  Just ( Constraints.mustBeSignedBy (tsSeller ts)
                                                    , State Nothing $
                                                      nft (negate 1)
                                                    )
    (v, p, Close Nothing)                           -> Just ( Constraints.mustBeSignedBy (tsSeller ts)
                                                    , State (p-p) $
                                                      v                                       <>
...
close ts _ = void $ mapErrorSM $ runStep (tsClient ts) $ Close -- Nothing
