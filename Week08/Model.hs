...
instance ContractModel TSModel where
    ...
    data Action TSModel =
            ...
            | Close Wallet Wallet (Maybe Integer)
        deriving (Show, Eq)
    ...
    data ContractInstanceKey TSModel w s e where
        ...
        -- EndKey   :: Wallet           -> ContractInstanceKey TSModel ()               TSEndSchema   Text
    ...
    arbitraryAction _ = oneof $
        ...
        [ Close     <$> genWallet <*> genWallet <*> genMaybeInt ]             ++
    ...
    -- nextState (Close w _ _) = do
    --     (tsModel . at w) $= Just (TSState 0 0 0)
    --     wait 1

    nextState (Close v w _) = do 
        when (v == w) $ do 
            m <- getTSState v
            case m of
                Just t ->
                    let 
                        tokenValue    = lovelaceValueOf $ t ^. tssLovelace
                        lovelaceValue = assetClassValue (tokens Map.! w) $ t ^. tssToken
                        -- nftValue      = nfts Map.! w
                    in do 
                        deposit w $ tokenValue <> lovelaceValue -- <> nftValue 
    ...
    perform h _ cmd = case cmd of
        ...
        (Close v w _)      -> callEndpoint @"close"      (h $ UseKey v w) Nothing                                              >> delay 1
    ...
    precondition s (Close v _ _)      = isJust    $ getTSState' s v
...
instanceSpec =
    ...
    -- [ContractInstanceSpec (EndKey w)   w endEndpoint  | w <- wallets]
...
genMaybeInt :: Gen (Maybe Integer)
genMaybeInt = arbitrary @(Maybe Integer) -- getNonNegative <$> arbitrary -- :: Gen (Maybe Integer)
