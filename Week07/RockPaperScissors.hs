...
transition game s r = case (stateValue s, stateData s, r) of
    ...
    (v, GameDatum _ (Just c'), Reveal _ c) 
        | lovelaces v == (2 * gStake game) && beats c c'
                                             -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)
                                                     , State Finished mempty
                                                     )
    (v, GameDatum _ (Just c'), Reveal _ c) 
        | lovelaces v == (2 * gStake game) && c==c' -- my impression is the NFT token gets handled by the state machine
                                             -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)       <>
    ...
    _                                        -> Nothing
--Constraints.mustPayToPubKey (gFirst game) (token <> lovelaceValueOf (2 * gStake game))
    ...
    case m of
        ...
        Just ((o, _), _) -> case tyTxOutData o of
            ...
            GameDatum _ (Just c') | c' == c -> do
                logInfo @String "second player played and tied"
                void $ mapError' $ runStep client $ Reveal (fpNonce fp) c
                logInfo @String "first player revealed and tied"
    ...
    case m of
        ...
        Just ((o, _), _) -> case tyTxOutData o of
            GameDatum _ Nothing -> do
                ...
                m' <- mapError' $ getOnChainState client
                case m' of
                    Nothing -> logInfo @String "first player won or tied" -- need sure how to disambiguate ties from losses here
                    Just _  -> do
