...
mkValidator dat () ctx | traceIfFalse "beneficiary's signature missing" signedByBeneficiary1 &&
                         traceIfFalse "deadline passed" deadlineAOK = True
                       | traceIfFalse "beneficiary's signature missing" signedByBeneficiary2 &&
                         traceIfFalse "deadline not yet reached" deadlineReached = True
                       | otherwise = False
  where
    info :: TxInfo
    ...
    deadlineAOK :: Bool
    deadlineAOK = after (deadline dat) $ txInfoValidRange info
